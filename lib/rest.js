...
Rest.prototype.putUser = function (req, res, next) {
  ...
  req.on('end', function () {
      ...
      deem.putUser(userId, user.email, user.password, function (obj) {
        console.log(obj);
        res.send(obj);
        next();
      });
...
Rest.prototype.putUserRating = function (req, res, next) {
  ...
  req.on('end', function () {
      ...
      deem.putUserRating(category, itemId, userId, rating.rating, function (obj) {
        console.log(obj);
        res.send(obj);
        next();
      });
...
Rest.prototype.getUserRating = function (req, res, next) {
  ...
  var userId = req.params.userId;
  this.deem.getUserRating(category, itemId, userId, function (obj) {
    console.log(obj);
    res.send(obj);
    next();
  });
